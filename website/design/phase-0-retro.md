...
The "Phase 0 is done when" criteria hold, all but CI on `main`, which comes with the merge's push.
The headless `free -g` baseline is recorded. The planted private address is refused on both
machines. `agent` can't read Dan's files or use Docker, and `nvidia-smi` run as `agent` names the
GPU; a real CUDA program as `agent` waits for Phase 1's first engine. The site builds, and CI passed
on the branch on 2026-09-24; Task 12's commits get their run at the next push.
...
Of the 68 commits, 45 came in the fix waves: 7 after the whole-branch review, and 38 in Task 12.
The ranges are git's `A..B`: the commits after `A`, up to and including `B`.
...
| The hooks came on only once Dan's denylist existed. | Without it they refuse every commit. Tasks 1–8 and their fix wave were committed with the hooks off and scanned by hand, then scanned again with the denylist, tree, history and messages, before Dan's OK to push. That scan is now a step before every push ([Leak guards](../how-to/leak-guards.md#before-every-push)). | the push gate, 2026-09-24 |
...
| Ubuntu's gitleaks 8.16 was removed once the plan's 8.30.1 release binary was in `/usr/local/bin`, rather than left on `PATH` beside it. | With both installed, which one ran depended on `PATH`. gitleaks has no snap, so the direct install stays, with its upgrade steps in `updates.md`. | `0c86fae`, `cb0ec0b` |
...
| The plan's code listings stay as written. Where a later fix changed the code, the listing is marked *Superseded*. | Correct, don't delete. The code in the repo is the as-built version. | `13633aa` |
...
1. **Plan code that was never run.** Phase 0's code listings went into the plan untested. The
   ...
   with nothing to check, and hold patterns that missed the kernel metapackages, the NVIDIA modules
   and 23 CUDA libraries. *Rule:* run every code block before it goes into a plan, and keep the
   listing byte-identical to the tested code. G3 worked this way for Phase 1's new code.
...
5. **Security came last.** The per-task reviews checked the spec and code quality. Security had a
   ...
   landed in the code and the runbooks first, and the box's side follows: a bootstrap re-run and
   three of Dan's steps (*Carried to Phase 1*). *Rule:* review permissions, secrets and network
   exposure in the task that changes them, before it reaches the box.
...
8. **Process slips.** One commit went in with another model's trailer and had to be amended. Shell
   ...
   Dan's OK; plans and runbooks are written with file tools, and a commit message with a body, or
   with backticks, `$` or `*` in it, goes through a file (`git commit -F`).
...
- **Pull before the next upgrade day.** Nothing after `96d0217` has been pushed, so the Spark's
  clone has the `updates.md` from before the council: no tmux step, no module check before the
  reboot, no GRUB check, a re-hold through the whole of `make bootstrap`, and a recovery line the
  reliability review showed can leave the box without a GPU. The current steps need `make hold-gpu`
  (`ed0e06a`) and bootstrap's `--hold-gpu` (`f77a144`), which that clone lacks. Once the merge is
...
- **Keys-only SSH** ([SSH from the Mac](../how-to/ssh.md#keys-only)), then its one-time public IPv6
  check, whose result goes in the vault.
...
- **Three facts to record:** Tailscale's version, the kernel's full release string, and the Spark's
  OpenSSH version (the keys-only drop-in was tested only with the Mac's).

Phase 1's Mac → Spark switch point checks keys-only SSH, the token and the secrets guard before any
**[Spark]** task, and before anything faces the network.

**Still unverified.** The plan's *To verify on the box* lists the design's open checks, and
`phase-1.md` and the runbooks mark the rest where they rely on one, as not yet checked, tried or
performed on this box. Among them: upgrade day's move of the GPU set and its recovery, untried until the first one; that
GRUB boots the newest kernel; and `agent` running a real CUDA program, which waits for Phase 1's
first engine.

**Deferred minors.** Small, and none blocks anything:

- There is no pre-push hook, so the scan before every push
  ([Leak guards](../how-to/leak-guards.md#before-every-push)) is run by hand.
...
- `secret-files.md` has no command that removes a key, so a leaked key has no documented way to be
  rotated. (Its other half, steps that refuse a key already there, was declined in `2381c21`: it
  would add a guard to seven commands, and the dedupe it would replace stays useful.)
...
**Decided, not deferred:** `publish-website.yml`'s checkout keeps the job token, because its publish
step pushes with it; `persist-credentials: false` is only in `ci.yml` (`bad1b55`).
...
denylist, `make hooks` and the drill), uv, Quarto for `make docs`, and a container runtime (Docker
Desktop here) for the throwaway `ubuntu:24.04` containers that test shell and apt behaviour.
`make test lint docs` must pass before anything else.
...
next. The code in the repo is the as-built version. In [Phase 0's plan](phase-0.md), a listing
whose code a later fix changed is marked *Superseded*; one that a later task extended as planned
isn't marked. Either way, the code wins.
